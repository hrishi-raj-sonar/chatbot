...
# loading the dataset
# Each tweet has a magnitude of its sentiment (0 to 1)
# Threshold of 0.5 is good enough to classify a tweet according to its sentiment.  
# setting each tweet's threshold magnitude accordingly
def load_training_data(sentiment):
    #data = open("C:/Users/hrish/Desktop/dataset/"+sentiment+"_training_set.txt",encoding="utf8")
    if sentiment == "anger":        
        threshold = 0.5
        data = open("anger_training_set.txt",encoding="utf8")
    elif sentiment == "fear":
        threshold = 0.6
        data = open("fear_training_set.txt",encoding="utf8")
    elif sentiment == "sadness":
        threshold = 0.5
        data = open("sadness_training_set.txt",encoding="utf8")
    elif sentiment == "joy":
        threshold = 0.5
        data = open("joy_training_set.txt",encoding="utf8")
    else:
        pass
...
def load_test_data(sentiment):
    data = open(sentiment+"_test_set.txt",encoding="utf8")
    return data


# cleaning the tweet data - removing punctuations and then tokenizing the words in tweet removing name tags
...
# collects all the unique words that are contained in the entire tweet dataset, finds their stem and 
...
# encode each tweet's words according to the words it contained from the bag of words which is based on all words in all tweets
...
def main():
    ...
    ### read the whole training data for each class and the THRESHOLD we will use for its classification
    ...
    ### read the whole test data for each class and the THRESHOLD we will use for its classification
...
#    print(joy_training_set[0][0],joy_training_set[0][1])
    ...
    ###combine all training sets in one list ###
    ...
    # convert our training,test set and training, test labels in a numpy array as it is required for calculations in neural net
    ...
    ###HERE WE HAVE A SHUFFLED DATASET WITH RESPECTIVE LABELS NOW WE HAVE TO TRAIN THESE BOTH NUMPY ARRAYS ###
